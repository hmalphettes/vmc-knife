    ...
    class RecipesConfigurationApplier
      ...
      def logs()
        ...
        if @root.wrapped['logs']
          ...
          name_prefix=man_logs['prefix']||"cflogs-"
          ...
          if parent_output_folder
            ...
            # clean the old files and folders if there are too many of them.
            max_number_logs_folders = 5
            log_folder_found_nb = 0
            Dir.new(parent_output_folder).entries.sort_by do |filename|
              File.mtime(File.join(parent_output_folder,filename))
            end.reverse_each do |filename|
              complete_path=File.join(parent_output_folder,filename)
              if filename =~ /#{name_prefix}/ && filename != "." && File.directory?(complete_path)
                log_folder_found_nb+=1
                if log_folder_found_nb >= max_number_logs_folders
